...
- **Migrate from S3** — Import buckets and objects from MinIO, AWS S3, or any S3-compatible source via a dashboard wizard (test connection → pick buckets → live progress, cancel in-flight jobs); **preserves each object's original modified date, user metadata, and content headers** (a faithful copy, not a same-day re-upload — something `mc mirror` can't do since it writes via PutObject); streams objects (no in-memory buffering) and retries transient errors, no AWS SDK required
