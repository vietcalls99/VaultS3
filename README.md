...
- **Migrate from S3** — Import buckets and objects from MinIO, AWS S3, or any S3-compatible source via a dashboard wizard (test connection → pick buckets → live progress, cancel in-flight jobs); streams objects (no in-memory buffering) and retries transient errors, no AWS SDK required
